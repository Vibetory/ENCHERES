...
public abstract class GenericJDBCDAOImpl<T> implements DAO<T> {
    ...
    public T update(T object) throws EException {
        String SQL_UPDATE = generateSQL_Update();
        try (Connection connection = JDBC.getConnection()) {
            PreparedStatement statement = connection.prepareStatement(SQL_UPDATE);
            prepareInsertOrUpdateStatement(statement, object, true);
            statement.executeUpdate();
            statement.close();
    ...
    public void delete(int... identifiers) throws EException {
        ...
        String SQL_DELETE = generateSQL_DELETE(fields.keySet());
        try (Connection connection = JDBC.getConnection()) {
            PreparedStatement statement = connection.prepareStatement(SQL_DELETE);
            generateStatementFields(statement, fields.values());
            statement.executeUpdate();
            statement.close();
        } catch (SQLException sqlException) {
            sqlException.printStackTrace();
     ...
     */
    protected T selectBy(String query, Collection<Object> fieldsValues) throws EException, SQLException {
        T instance = null;
        Connection connection = JDBC.getConnection();
        PreparedStatement statement = connection.prepareStatement(query);
        generateStatementFields(statement, fieldsValues);
    ...
    public List<T> selectAll() throws EException {
        ...
        String SQL_SELECT_ALL = generateSQL_SELECT_ALL();
        try (Connection connection = JDBC.getConnection()) {
     ...
     * @throws SQLException SQLException
     */
    protected List<T> selectALlBy(String query, Collection<Object> fieldsValues) throws EException, SQLException {
        List<T> instances = new ArrayList<>();
        Connection connection = JDBC.getConnection();
        PreparedStatement statement = connection.prepareStatement(query);
        generateStatementFields(statement, fieldsValues);
     ...
     */
    private void prepareInsertOrUpdateStatement(PreparedStatement statement, T object, boolean update) throws EException {
        try {
     ...
     * @throws EException EException | GENERATE_STATEMENT_DATA_ERROR.
     */
    private void generateStatementFields(PreparedStatement statement, Collection<Object> fieldsValues) throws EException {
        try {
            int parameterIndex = 1;
    ...
    private String getFieldsSelection(Set<String> fields, boolean isUnknownParameter, boolean isConditions, boolean isUpdate) {
        ...
        for (String field : fields) {
            ...
            separator = isConditions ? " AND " : ", ";
        }
        return fieldsSelection.toString();
    }

    private String getFieldsSelection() {
        return getFieldsSelection(this.fields.keySet(), false, false, false);
    }
    ...
    private String generateSQL_SELECT(Set<String> fields) {
        return "SELECT " + getFieldsSelection() + " FROM " + getActualClassName() + " WHERE " + getFieldsSelection(fields, true, true, false);
    }

    private String generateSQL_SELECT_ALL() {
        return "SELECT " + getFieldsSelection() + " FROM " + getActualClassName();
    }

    private String generateSQL_DELETE(Set<String> fields) {
        return "DELETE FROM " + getActualClassName() + " WHERE " + getFieldsSelection(fields, true, true, false);
    }

    private String generateSQL_Update() {
        return "UPDATE " + getActualClassName() + " SET " + getFieldsSelection(this.fields.keySet(), true, false, true) + " WHERE " + getFieldsSelection(new LinkedHashSet<>(this.identifiers), true, true, false);
    }

    private String generateSQL_INSERT() {
        StringBuilder unknownParameters = new StringBuilder();
        String separator = "";
        int length = this.fields.size() - this.autogeneratedIdentifiers;
        while (length > 0) {
            unknownParameters.append(separator).append("?");
            separator = ", ";
            length --;
        }
        return "INSERT INTO " + getActualClassName() + " VALUES ( " + unknownParameters.toString() + " ) ";
    }
}
