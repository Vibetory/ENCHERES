...
public abstract class GenericJDBCDAOImpl<T> implements DAO<T> {
     ...
     * @param update boolean | "true" if the operation is an update, "false" if it is an insert.
     * @return T | Instance of the actual object successfully inserted or updated into the database.
     * @throws EException
     */
    private T insertOrUpdate(T object, String query, boolean update) throws EException {
        try (Connection connection = JDBC.getConnection()) {
            PreparedStatement statement = connection.prepareStatement(query, PreparedStatement.RETURN_GENERATED_KEYS);
            setStatementParameters(statement, object, update);
     ...
     */
    private void setStatementParameters(PreparedStatement statement, T object, boolean update) throws EException {
        try {
    ...
    private String generateQueryFields(Set<String> fields, boolean isUnknownParameter, boolean isConditions, boolean isUpdate) {
    ...
    private String generateQueryFields() {
        return generateQueryFields(this.fields.keySet(), false, false, false);
    }
    ...
    private String generateSQL_SELECT(Set<String> fields) {
        return "SELECT " + generateQueryFields() + " FROM " + getActualClassName() + " WHERE " + generateQueryFields(fields, true, true, false);
    }

    private String generateSQL_SELECT_ALL() {
        return "SELECT " + generateQueryFields() + " FROM " + getActualClassName();
    }

    private String generateSQL_DELETE(Set<String> fields) {
        return "DELETE FROM " + getActualClassName() + " WHERE " + generateQueryFields(fields, true, true, false);
    }

    private String generateSQL_UPDATE() {
        return "UPDATE " + getActualClassName() + " SET " + generateQueryFields(this.fields.keySet(), true, false, true) + " WHERE " + generateQueryFields(new LinkedHashSet<>(this.identifiers), true, true, false);
    }
